...
namespace DynamoDB.Geo.Contract.Models
{
    public sealed class GeohashRange
    {
        ...
        {
            if (range.RangeMin - RangeMax <= MergeThreshold
            ...
            if (RangeMin - range.RangeMax <= MergeThreshold
        ...
        public IReadOnlyList<GeohashRange> TrySplit(int hashKeyLength)
        {
            ...
            var minHashKey = S2Utils.GenerateHashKey(RangeMin, hashKeyLength);
            var maxHashKey = S2Utils.GenerateHashKey(RangeMax, hashKeyLength);
